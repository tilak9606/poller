...
export async function validatePollAnswers(pollId, answers) {
    ...
    for (const answer of answers) {
        ...
        const optionExists = question.options.some((option) => option._id.toString() === answer.selectedOption);
        if (!optionExists) {
            throw ApiError.badRequest("Invalid selected option");
        }
    }
    const answeredQuestionIds = new Set(answers.map((answer) => answer.question));
    const missingRequiredQuestion = questions.find((question) => question.isRequired &&
